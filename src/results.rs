...
pub fn parse_test_results(stdout: &str) -> Vec<Event> {
    stdout.lines().map(serde_json::from_str).map(Result::unwrap).collect()
}
...
mod tests {
    ...
    fn suite_started() {
        ...
        let expected = Event::new_suite(EventKind::Started)
            .set_test_count(1);
...
{ "type": "suite", "event": "ok", "passed": 1, "failed": 0, "allowed_fail": 0, "ignored": 0, "measured": 0, "filtered_out": 40 }"#;
        ...
        expected.push(Event::new_test(EventKind::Started, String::from("parsers::test::test_zpools_on_single_zpool")));
        expected.push(Event::new_test(EventKind::Ok, String::from("parsers::test::test_zpools_on_single_zpool")));
        expected.push(Event::new_suite(EventKind::Ok)
            .set_passed(1)
            .set_failed(0)
            .set_allowed_fail(0)
            .set_ignored(0)
